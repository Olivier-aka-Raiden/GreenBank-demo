...
;( function( window ) {
	...
	function displayWelcome(self) {
		if (self.current > 0) {
			...
			if (userName) {
				// If the user has provided a name, display the welcome message
				welcomeMessage.classList.remove('hidden');

				// Update the label text with the user's name
				const label = welcomeMessage.querySelector('label');
				label.textContent = `Bienvenue ${userName}, \n Procédons à votre simulation...`;
			} else {
				// If the name input is empty, hide the welcome message
	...
	FForm.prototype._initEvents = function () {
		...
		this.ctrlContinue.addEventListener( 'click', function() {
			// Check if the current field is the second question (q2)
			displayWelcome(self);
			self._nextField();
		} );
		...
		this.fields.forEach( function( fld ) {
			if( fld.hasAttribute( 'data-input-trigger' ) ) {
				...
				switch( input.tagName.toLowerCase() ) {
					case 'select' : 
						input.addEventListener( 'change', function() { displayWelcome(self); self._nextField(); } );
						break;

					case 'input' : 
						[].slice.call( fld.querySelectorAll( 'input[type="radio"]' ) ).forEach( function( inp ) {
							inp.addEventListener( 'change', function(ev) { displayWelcome(self); self._nextField(); } );
						} ); 
						break;
				}
			}
